...
class BPlusTree_Correct_Update
{
private:
	...
	{
		int i = _BinarySearchInKeys(node, MiddleKey);

		typename vector <K>::iterator it = node->vKeys.begin() + i;
	...
	int _BinarySearchInKeys(Node* node, K& key)
	{
		int first = 0;
		int last;
	...
	void _Insert(Node* node, K key, T Data)
	{
		if (node->isLeaf)
		{
			...
			class vector <NodeOfKeysAndData>::iterator it = node->vKeysAndData.begin() + i;
...
public:
	...
	BPlusTree_Correct_Update(int Order)
	{
		float m = Order;
	...
	void Copy(BPlusTree_Correct_Update& CopyBPlusTree_Correct_Update)
	{
		if (!IsEmpty())
		{
			cout << "\nTree Must be Empty!\n";
		...
		if (CopyBPlusTree_Correct_Update.IsEmpty())
		{
			cout << "\nCopy Tree is Empty!\n";
			return;
		}

		if (CopyBPlusTree_Correct_Update._MaxChildren != _MaxChildren)
		{
			cout << "\nThe Order in two Btree must be the same!\n";
			return;
		}

		_Copy(CopyBPlusTree_Correct_Update._Root);
	}

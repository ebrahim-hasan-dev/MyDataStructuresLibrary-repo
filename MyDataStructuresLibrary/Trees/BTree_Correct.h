...
private:
	...
	LeftAndRightChildren _Insert3(Node* node, NodeOfKeyAndData Middleitem, int& pos)
	{
		...
		if (Middleitem.key < node->vKeysAndData[0].key)
		{
			it = node->vKeysAndData.begin();
		...
		else if (Middleitem.key > node->vKeysAndData.back().key)
		{
			Node* tmp = nullptr;
			pos = node->vKeysAndData.size();
		...
		else
		{
			...
			{
				if (Middleitem.key > node->vKeysAndData[i].key && Middleitem.key < node->vKeysAndData[i + 1].key)
				{
	...
	int _Insert2(Node* node, NodeOfKeyAndData Middleitem)
	{
		...
		{
			int indexLstItem = node->vKeysAndData.size();
			Node* tmp = nullptr;

			node->vKeysAndData.push_back(Middleitem);
			node->vChildren.push_back(tmp);

			Indexitem = indexLstItem;
		}
		else
		{
			for (int i = 0; i < node->vKeysAndData.size() - 1; i++)
	...
	void _Insert(Node* node, double key, T Data)
	{
		if (node->isLeaf)
		{
			class vector <NodeOfKeyAndData>::iterator it;
			...
			{
				node->vKeysAndData.push_back(NodeOfKeyAndData(key, Data));
			}
			else
			{
				for (int i = 0; i < node->vKeysAndData.size() - 1; i++)
...
public:
	...
	void Copy(BTree_Correct& CopyBTree_Correct)
	{
		if (!IsEmpty())
		{
			cout << "\nTree Must be Empty!\n";
		...
		if (CopyBTree_Correct.IsEmpty())
		{
			cout << "\nCopy Tree is Empty!\n";
			return;
		}

		if (CopyBTree_Correct._MaxChildren != _MaxChildren)
		{
			cout << "\nThe Order in two Btree must be the same!\n";
			return;
		}

		_Copy(CopyBTree_Correct._Root);
	}
